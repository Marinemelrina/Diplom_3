...
class OrdersPageLocators:
    ORDERS_LIST = (By.XPATH, '//h1[text()="Лента заказов"]')  # заголовок "Лента заказов"
    ORDER_LINK = (By.XPATH, '//*[contains(@class, "OrderHistory_link")]')  # ссылка на заказ в списке "Лента заказа"
    ORDER_STRUCTURE = (By.XPATH, '//p[text()="Cостав"]')  # заголовок "Состав" в окне с деталями заказа
    ORDER_NUMBER = (By.XPATH, '//p[text()="{}"]')  # номер заказа
    CURRENT_ORDERS_READY = (By.XPATH, '//li[text()="Все текущие заказы готовы!"]')  # текст "Все текущие заказы готовы!"
